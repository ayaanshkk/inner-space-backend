...
# 🔑 FIX: Define the logger using standard Python logging ONLY.
...
class DrawingAnalyzer:
    def __init__(self):
        ...
        # NOTE: Using the standard logger defined globally above
        self.logger = logger
    ...
    def _init_google_vision_client(self):
        try:
            if GOOGLE_CLOUD_CREDENTIALS and GOOGLE_CLOUD_CREDENTIALS.strip().startswith('{'):
                self.logger.debug("Initializing Google Vision client from inline JSON credentials.")
                creds_dict = json.loads(GOOGLE_CLOUD_CREDENTIALS)
                credentials = service_account.Credentials.from_service_account_info(creds_dict)
                client = vision.ImageAnnotatorClient(credentials=credentials)
                self.logger.info("Google Vision client initialized from inline JSON credentials.")
                return client

            if GOOGLE_CLOUD_CREDENTIALS and os.path.exists(GOOGLE_CLOUD_CREDENTIALS):
                self.logger.debug("Initializing Google Vision client from credentials file path.")
                with open(GOOGLE_CLOUD_CREDENTIALS, 'r', encoding='utf-8') as fh:
                    creds_dict = json.load(fh)
                credentials = service_account.Credentials.from_service_account_info(creds_dict)
                client = vision.ImageAnnotatorClient(credentials=credentials)
                self.logger.info("Google Vision client initialized from JSON file.")
                return client

            if os.getenv('GOOGLE_APPLICATION_CREDENTIALS'):
                self.logger.debug("GOOGLE_APPLICATION_CREDENTIALS detected.")
                client = vision.ImageAnnotatorClient()
                self.logger.info("Google Vision client initialized using GOOGLE_APPLICATION_CREDENTIALS path.")
                return client

            self.logger.debug("Attempting to initialize Vision client using default credentials (ADC).")
            client = vision.ImageAnnotatorClient()
            self.logger.info("Google Vision client initialized using default credentials (ADC).")
            return client

        except Exception as e:
            self.logger.warning(f"Could not initialize Google Cloud Vision client: {e}")
            return None
    ...
    def extract_numbers_with_google_vision(self, image_bytes):
        if not self.vision_client:
            self.logger.error("Google Cloud Vision client not initialized")
            return None, []
        
        try:
            self.logger.info("Extracting text with Google Cloud Vision...")
            ...
            self.logger.debug(f"Raw Google Vision response (truncated): {json.dumps(raw_resp, indent=2)[:2000]}")
            ...
            self.logger.info(f"Extracted {len(extracted_numbers)} numbers from image")
            ...
            self.logger.error(f"Error in Google Cloud Vision extraction: {str(e)}")
            return None, []
    
    def extract_numbers_with_openai_vision(self, image_bytes):
        """Fallback method: Extract numbers directly using OpenAI Vision"""
        try:
            self.logger.info("Using OpenAI Vision as fallback for text extraction...")
            ...
            if json_start != -1 and json_end > json_start:
                ...
                self.logger.info(f"OpenAI Vision extraction succeeded: {len(dimension_analysis.get('all_numbers', []))} numbers found")
            ...
            self.logger.error(f"OpenAI Vision fallback failed: {str(e)}")
            return None, {}
    ...
    def analyze_with_master_prompt(self, image_bytes, extracted_numbers_data):
        ...
        *WIDTH IDENTIFICATION PRIORITY (CRITICAL):*
        ...
        try:
            self.logger.info("Analyzing with OpenAI GPT-4 Vision...")
    ...
    def analyze_technical_drawing(self, image_bytes):
        self.logger.info("Starting kitchen cabinet technical drawing analysis with Google Cloud Vision + OpenAI")
        ...
        try:
            ...
            if not dimension_analysis or not dimension_analysis.get('all_numbers'):
                self.logger.warning("Google Cloud Vision extraction failed or returned no data, using OpenAI Vision fallback")
                full_text, dimension_analysis = self.extract_numbers_with_openai_vision(image_bytes)
            
            if not dimension_analysis or not dimension_analysis.get('all_numbers'):
                self.logger.error("Failed to extract numbers from image using both methods")
            ...
            if 'error' in analysis_result:
                self.logger.error(f"Analysis error: {analysis_result['error']}")
                return self.generate_empty_cutting_list()
            ...
            if total_components == 0:
                self.logger.error("No valid components generated")
                return self.generate_empty_cutting_list()
            
            self.logger.info(f"Analysis complete: Generated {total_components} components")
    ...
    def process_cabinet_analysis(self, analysis):
        ...
        if not cabinet_modules:
            self.logger.error("No cabinet modules identified")
            return
        ...
        if not self.validate_cabinet_dimensions(cabinet_width, cabinet_working_height, cabinet_depth):
            self.logger.error(f"Invalid cabinet dimensions: {cabinet_width}×{cabinet_working_height}×{cabinet_depth}")
            return
        
        self.logger.info(f"Processing kitchen cabinet: {cabinet_width}W × {cabinet_total_height}H (working: {cabinet_working_height}H) × {cabinet_depth}D mm")
        ...
        for comp_name, (category, description) in component_mapping.items():
            if comp_name not in calculated_components:
                self.logger.warning(f"Missing component: {comp_name}")
                continue
            ...
            if not self.validate_component_dimensions(height, width, comp_name):
                self.logger.warning(f"Invalid dimensions for {comp_name}: {height}×{width}")
                continue
            ...
            self.logger.info(f"Added {description}: {height}×{width}mm (Qty: {quantity})")
    ...
    def generate_cutting_list(self):
        ...
        self.logger.info(f"Generated {total_items} total components")
        return summary
